...
class MessageManager {
    ...
    public function newMessage($thread, $user, $messageText) {
            ...
            if($messageText == null || trim($messageText) == ""){
                throw new GameException("USER.NEW.MESSAGE.WRONG.PARAMETERS");
            }
    ...
    public function newThread($subject, $creatorUser, $recipientUsersArray, $returnOldThreadIfExisting) {
            ...
            if(strlen($subject) < 6 || strlen($subject) > 32){
                throw new GameException("NO.VALID.SUBJECT.SPECIFIED");
            }
            if(count($recipientUsersArray) < 1){
                throw new GameException("NO.USERS.SELECTED");
            }
